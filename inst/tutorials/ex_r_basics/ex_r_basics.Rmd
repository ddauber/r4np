...
```{r book-list-hint-1, eval=FALSE}
# Create an object called 'books'
books <- ...
```

```{r book-list-hint-2, eval=FALSE}
...
```{r todo-hint-1, eval=FALSE}
# Create an object 'todo'
todo <- ...
```

```{r todo-hint-2, eval=FALSE}
# Use the function 'c()'
todo <- c(...)
```
...
```{r nested-list-hint-2, eval=FALSE}
# Option 1: You use '$' after the object name to dig deeper into the list
file_daniel$Food$
```
